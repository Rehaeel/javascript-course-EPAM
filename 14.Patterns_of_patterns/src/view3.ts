...
export class View {
    ...
    private isViewA = (view: "a" | "b"): boolean => view === "a";
    private isModelA = (): boolean => this.model === "modelA";

    public setView(view: "a" | "b") {
        this.render(view);
    }

    public render(view: "a" | "b") {
        container.innerHTML = database.reduce((acc, curr) => {
            ...
            const exchangeOutput: string | number = this.isViewA(view)
                ? `<input type="number" step="0.05" value="${curr.exchangeRate}"/>`
                : curr.exchangeRate;

            const euroOutput = (): string => {
                const outputValue: number = this.isModelA()
                    ? ModelA.instance().value
                    : curr.baseCurrAmount;

                return `<label> EURO <span id="euro-output-${curr.title}">${
                    this.isViewA(view) ? "" : outputValue
                }</span>
                        <input class="euro-amount"  ${
                            this.isViewA(view)
                                ? `type="number"`
                    ...
                    this.isViewA(view)
                        ...
                        <input ${
                            this.isViewA(view)
                                ? `type="number"`
                                : `type="range" min="0" max="10000" `
                        }class="converted-amount" value="${curr.convertedCurrAmount.toFixed(
                    2
                )}"/>
                    </label>`;
            };
        ...
        <h3>${curr.title}</h3>
        <label class="exchange-rate">1 EUR is ${exchangeOutput} ${
                curr.title
            }</label>
